...
class HiPAMA(nn.Module):
    def __init__(self, embed_dim, depth, input_dim=84, num_heads=4, phn_vocab_size=40):
        super().__init__()
        self.input_dim = input_dim
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.conv_dim = 25
        self.phn_vocab_size = int(phn_vocab_size)

        # phone projection
        self.phn_proj = nn.Linear(self.phn_vocab_size, embed_dim)
    ...
    def forward(self, x, phn):
        ...
        phn_one_hot = torch.nn.functional.one_hot(phn.long()+1, num_classes=self.phn_vocab_size).float()
